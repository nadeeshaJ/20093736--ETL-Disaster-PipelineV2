...
def fetch_and_process():
    ...
    if live_df.empty: return None, None, {"refreshed": "Sync Required"}

    # Numeric and Triage Logic Fixes
    ...
    # Recalculating for scientific consistency
    live_df["triage_score"] = ((live_df["real_time_severity_score"] / 3 * 35) + 
                               (live_df["vulnerability_index"].clip(upper=100) * 0.25) + 
                               (live_df["impact_score"].clip(upper=100) * 0.25) + 
                               ((live_df["funding_gap_usd"].clip(upper=1e8)/1e8)*100 * 0.15)).round(2)
    live_df["urgency_level"] = pd.cut(live_df["triage_score"], bins=[-0.1, 30, 60, 85, 105], labels=["Routine", "Heightened", "Urgent", "Emergency"])

    kpis = {
        "total": len(live_df), "high_risk": (live_df['vulnerability_index'] > 50).sum(),
        "gap_b": live_df['funding_gap_usd'].sum() / 1e9,
        "refreshed": live_df["processed_at"].iloc[0] if "processed_at" in live_df.columns else "Manual",
        "hist_refreshed": hist_df["processed_at"].iloc[0] if not hist_df.empty and "processed_at" in hist_df.columns else "N/A"
    }
...
def index():
    ...
    # 4. RESILIENCE QUADRANTS
    fig_res = px.scatter(live_df, x="vulnerability_index", y="electricity_access_pct", size="estimated_required_aid_usd", color="urgency_level", hover_name="country", text="iso3")
