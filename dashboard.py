...
# ---------------------------------------------------------
# DATABASE CONNECTION & AUDIT SYNC
# ---------------------------------------------------------
...
def fetch_and_audit():
    """Retrieves data and extracts the exact Pipeline Run timestamp."""
    try:
        ...
        if live_df.empty:
            return pd.DataFrame(), pd.DataFrame(), {"refreshed": "Sync Required"}

        # Extracting the actual Pipeline Stamp from the data
        pipeline_time = live_df["processed_at"].iloc[0] if "processed_at" in live_df.columns else "Manual Load"
        hist_time = hist_df["processed_at"].iloc[0] if not hist_df.empty and "processed_at" in hist_df.columns else "N/A"

        # Applying scientific logic fixes for consistent reporting
        ...
        # Simplified/Balanced Triage Logic
        ...
        kpis = {
            ...
            "hist_refreshed": hist_time
        }
        return live_df, hist_df, kpis
    except Exception as e:
        print(f"❌ Database Sync Error: {e}")
        return pd.DataFrame(), pd.DataFrame(), {"refreshed": "Error"}

@app.route("/")
def index():
    live_df, hist_df, kpis = fetch_and_audit()

    # 1. GDACS-Style Satellite Monitor
    fig_map = px.scatter_mapbox(live_df, lat="latitude", lon="longitude", color="disaster_type",
                                ...
                                mapbox_style="satellite-streets", # Professional satellite view
                                color_discrete_sequence=px.colors.qualitative.Bold,
                                size_max=30, zoom=1.1, height=650)
    ...
    fig_acc = px.scatter(hist_df, x="total_deaths", y="estimated_deaths", log_x=True, log_y=True, opacity=0.6,
                         title="<b>Scientific Accuracy (Log-Scale)</b>")
